...
//   node scripts/render.mjs [--frames=0-89] [--scale=0.5] [--browser=/path/to/chrome-headless-shell]
...
const composition = await selectComposition({ serveUrl, id: 'EngineDemo', browserExecutable });
...
for (const frame of stills) {
  await renderStill({ composition, serveUrl, frame, scale, browserExecutable, output: join(root, 'out', `frame-${frame}.png`) });
