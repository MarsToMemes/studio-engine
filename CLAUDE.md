...
- **Studio blocks** (`packages/remotion/hyperframes-studio/`, SCENE_ENGINE.md §28.1) are our Apple-style HyperFrames compositions: edit them there, keep them on the `_studio` design system, sync, and check with `hyperframes:probe --only <name>` and a still sheet before rendering.
